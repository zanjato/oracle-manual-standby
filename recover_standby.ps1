...
'@
  ...
  function sb_cn{
    ...
    $cm.connection=$sboc
    $cm.commandtext=@'
select min(current_scn) as scn
from (select checkpoint_change# as current_scn from v$datafile_header
       union all
      select checkpoint_change# as current_scn from v$datafile
       union all
...
'@ -replace $my.LNW,$my.NL.str
    ...
    $cm.commandtext=@"
  /*select bp.handle name,bad.next_change# xcn
  from v`$backup_archivelog_details bad
  join v`$instance i
    on bad.thread# = i.thread#
  join v`$backup_piece bp
    on bad.id1 = bp.set_stamp and bad.id2 = bp.set_count
  where bad.btype='BACKUPSET' and bad.resetlogs_change#=$($pr.rlch) and
    bad.resetlogs_time=to_date('${rltm}','yyyymmddhh24miss') and
    bad.next_change#>$($sbcn+1)
union all*/
  select a.name, a.next_change# xcn
