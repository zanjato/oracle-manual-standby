...
#get-acl (join-path $env:systemroot\system32\tasks manual_standby_recovery.job)|
#  set-acl (join-path $env:systemroot\system32\tasks backup_oracle.job)
#powershell.exe -nol -nop -non ^
#  -f C:\oracle\product\10.2.0\admin\EKR\standby\backup_oracle.ps1
param([switch]$call)
...
<#        $rd=$cm.executereader('singleresult,singlerow')
    ...
    for(;;){
      $out|?{$_ -match '\(\s*host\s*=\s*([\w.]*[\w])\s*\)'}|%{break}
